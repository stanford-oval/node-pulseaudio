...
function createStream(ctx, self, opts, type){
  ...
  var stm = self.$ = new PulseStream(ctx.$, str2num(opts.format, PulseStream.format), opts.rate, opts.channels, opts.stream, function streamState(state, error){
...
PlaybackStream.prototype._write = function(chunk, encoding, done){
  return this._connection(function playbackWrite(){
    if(this.playing){
      this.$.write(chunk, function(){ done(); });
    }else{
      done();
    }
  });
...
PlaybackStream.prototype.stop = function(){
  ...
  for(; this._writableState.buffer.shift(); );
